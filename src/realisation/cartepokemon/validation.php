...
if (empty($_SESSION["uti"]) or empty($_SESSION["pw"])){
	session_destroy();
	print "<script language='JavaScript'>self.location.href='index.php?erreur=0'</script>";
}	
else{
	...
	if(autorisation($_SESSION["uti"],$_SESSION["pw"]))
		...
		session_destroy();
		echo "<script language='JavaScript'>self.location.href='index.php?erreur=1'</script>";
	}		
}
?>
