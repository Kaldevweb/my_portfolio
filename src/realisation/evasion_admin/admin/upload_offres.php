          ...
          <?php
               ...
               else
               {
                    ...
                    if(autorisation($_SESSION["uti"],$_SESSION["pw"]))
                    {
                         ...
                         @$new_offre_titre=$_POST["new_offre_titre"];
                         $new_offre_titre = $bdd -> real_escape_string($new_offre_titre);


                         @$new_offre_description=$_POST["new_offre_description"];
                         $new_offre_description = $bdd -> real_escape_string($new_offre_description);
...

                         {
                              ...
                              if(empty($new_offre_titre) OR empty($new_offre_description) OR empty($amount))
...

                              {
...

                                   {
                                        ...
                                        $req_upload = $bdd->query("INSERT INTO evasion_admin_article_offre VALUES('','$new_offre_titre','$new_offre_description','$amount', '$fichier')") or die($bdd->error);

                                        //CREATION DU THUMBS
                                        $name="../img/offres/grd/$fichier"; //------Recuperer le chemin de l'image original pour que on puisse faire un thumbs avec
                                        $filename="../img/offres/$fichier";
                                        createthumb($name,$filename,300,300); //-------création du thumnail
