...
else
{
	...
	if(autorisation($_SESSION["uti"],$_SESSION["pw"]))
	{
		...
		$query_image = $bdd -> query("SELECT * FROM evasion_admin_slider WHERE id='$id_sup_slider'") or die($bdd->error);
		$val = $query_image->fetch_array();
		$fichier = "../../img/slider/$val[image]";		

		//SUPPRESSION DE L'ARTICLE
		$sup = $bdd ->query("DELETE FROM evasion_admin_slider WHERE id='$id_sup_slider'")or die($mysqli->error);
