...
else
{
	...
	if(autorisation($_SESSION["uti"],$_SESSION["pw"]))
	{
		...
		$mod_titre_offre = $bdd -> real_escape_string($mod_titre_offre);

		@$mod_description_offre=$_POST["mod_description_offre"];
		$mod_description_offre = $bdd -> real_escape_string($mod_description_offre);
		...
		//UPDATE DE L'ARTICLE
		$update = $bdd -> query("UPDATE evasion_admin_article_offre SET titre='$mod_titre_offre', description='$mod_description_offre', prix='$mod_prix_offre' WHERE id='$mod_article_offre'") or die($bdd->error);
