...
else
{
	...
	if(autorisation($_SESSION["uti"],$_SESSION["pw"]))
	{
		...
		$mod_titre_article = $bdd -> real_escape_string($mod_titre_article);

		@$mod_description_article=$_POST["mod_description_article"];
		$mod_description_article = $bdd -> real_escape_string($mod_description_article);

		@$mod_source_article=$_POST["mod_source_article"];
		$mod_source_article = $bdd -> real_escape_string($mod_source_article);
		...
		//UPDATE DE L'ARTICLE
		$update = $bdd -> query("UPDATE evasion_admin_article_destination SET titre='$mod_titre_article', description='$mod_description_article', source='$mod_source_article', pays='$modif_article_pays' WHERE id='$id_modif_article'") or die($bdd->error);
