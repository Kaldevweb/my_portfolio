...
else
{
	...
	if(autorisation($_SESSION["uti"],$_SESSION["pw"]))
	{
		...
		$query_image = $bdd -> query("SELECT * FROM evasion_admin_article_offre WHERE id='$id_offre_sup'") or die($bdd->error);
		$val = $query_image->fetch_array();
		$fichier = "../../img/offres/grd/$val[image]";
		$fichier_thumbs = "../../img/offres/$val[image]";		

		//SUPPRESSION DE L'ARTICLE
		$sup = $bdd ->query("DELETE FROM evasion_admin_article_offre WHERE id='$id_offre_sup'")or die($mysqli->error);
