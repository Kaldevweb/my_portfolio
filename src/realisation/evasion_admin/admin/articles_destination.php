		...
		$('#btn_supp_article_destination').click(function(){
			...
			if(confirmation){
		    	$.ajax({
					...
					success:function(data){
						$('#mess_validation').html(data);
						$('#sup_article_destination :selected').remove();
					}	//FIN SUCCES
		        }); //FIn AJAX
		    }
...
<div id="container">
	...
	<div id="boite_separation">
		...
		<select name="new_article_pays" id="new_article_pays">
			...
			<?php
				...
				$query = $bdd -> query("SELECT DISTINCT pays FROM evasion_admin_article_destination ORDER BY pays") or die($bdd->error);
	...
	<div id="boite_separation">
		...
		<?php
			...
			$query = $bdd -> query("SELECT * FROM evasion_admin_article_destination ORDER BY pays") or die($bdd->error);
