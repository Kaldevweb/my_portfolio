...
else
{
	...
	if(autorisation($_SESSION["uti"],$_SESSION["pw"]))
	{
		...
		$query_image = $bdd -> query("SELECT * FROM article_destination WHERE id='$id_article_sup'") or die($bdd->error);
		$val = $query_image->fetch_array();
		$fichier = "../../img/destination/$val[image_petite]";		

		//SUPPRESSION DE L'ARTICLE
		$sup = $bdd ->query("DELETE FROM article_destination WHERE id='$id_article_sup'")or die($mysqli->error);
