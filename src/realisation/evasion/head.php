...
<header>
	...
	<div id="cssmenu">
		...
		$query_menu = $bdd -> query("SELECT * FROM evasion_menu WHERE parent = 0 ORDER BY positionMenu") or die($bdd->errno);
...

		{
			...
			$query_sousmenu = $bdd -> query("SELECT * FROM evasion_menu WHERE parent='$val[id]' ORDER BY positionMenu") or die($bdd->errno);
	...
	<div class="picSlider demo-2">
		...
		<ul>
			...
			<?php
				...
				$query = $bdd -> query("SELECT * FROM evasion_slider ORDER BY id") or die($bdd->error);
